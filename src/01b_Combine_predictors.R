...
## Scale predictors ####
# scale rasterStack
Env_norm <- terra::scale(Env)
Env_norm <- terra::stack(Env_norm)

# save Env_norm
terra::writeRaster(Env_norm, file="D:/_students/Romy/SoilBiodiversity/results/EnvPredictor_2km_normalized.grd", overwrite=T)

# same for dataframe
Env_norm_df <- as.data.frame(Env_norm)
...
files <- list.files(paste0(here::here(), "/data_environment"), 
                    include.dirs = F, recursive=F, full.names = T)
files <- files[str_detect(files, "5km")]
files
...
# as dataframe
Env <- terra::rast("D:/_students/Romy/SoilBiodiversity/results/EnvPredictor_5km.grd")
Env_df <- as.data.frame(Env, xyz=TRUE, row.names=FALSE)
...
# save Env_norm
terra::writeRaster(Env_norm, file="D:/_students/Romy/SoilBiodiversity/results/EnvPredictor_5km_normalized.grd", overwrite=T)
