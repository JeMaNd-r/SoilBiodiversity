...
View(count_data %>% arrange(desc(CleanOcc), SpeciesID))
```
...
count_data <- count_data %>% full_join(speciesNames %>%
                                         dplyr::select(-NumCells_2km, -Group_name), by=c("species" = "Species", "Acc_name", "SpeciesID", "Species_final"))
count_data

ggplot(count_data %>% filter(!is.na(SpeciesID), Included=TRUE), aes(x=RawOcc-CleanOcc, y=SpeciesID, fill=Ecogroup))+
...
# edapho 13054, gbif 60219, jean 24860, jerome 732, soilrecon 175, swowm 5908
```
...
plotOcc <- ggplot()+ 
  ...
  scale_color_gradient2(5,    # provide any number of colors
    ...
    breaks = c(5, 20, 50, 100, 200, 300), limits=c(0,300))+
  theme_bw()+
