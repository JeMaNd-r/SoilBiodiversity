...
for(no.runs in 1:no.loop.runs){
  ...
  rm(temp_prediction, mars_varImp, mars_fit, temp_validation, temp_model_time, temp_predict_time, mytuneGrid, mytrControl)
}
...
foreach(spID = unique(speciesNames[speciesNames$NumCells_2km >= 5,]$SpeciesID)[1:10], 
         .export = c("Env_norm", "Env_norm_df", "form"),
         .packages = c("tidyverse","biomod2", "caret", "xgboost")) %dopar% { try({
