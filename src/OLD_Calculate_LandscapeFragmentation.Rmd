...
landscape_summary <- lapply(1:length(poly2k), function(x) { try({
  temp_map <- raster::crop(clc12, extent(poly2k[x,]))
  
  # Calculate metrics
  point_summary <- as.data.frame(calculate_lsm(temp_map,
  ...
  point_summary <- as.data.frame(tidyr::pivot_wider(point_summary, names_from=metric, values_from=value))
...
save(landscape_summary, file="D:/00_datasets/LandCover/V025_Land_Frag/Land_Frag_CLC_2km_lsmPerPolygon.RData")
