...
```{r}
# 2km
poly2k <- as(grid2k, 'SpatialPolygons')
...
for(j in 1:length(poly2k)){
  maps[[j]] <- NA
  try(maps[[j]] <- raster::crop(clc12, extent(poly2k[j,])), silent=T)

  names(maps)[[j]] <- paste0("map","_", j)
}

save(maps, file="D:/00_datasets/LandCover/V025_Land_Frag/Land_Frag_CLC_polygonPerCell.RData")
