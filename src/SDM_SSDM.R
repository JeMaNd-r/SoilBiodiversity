...
# crop to Europe
for(i in 1:nlayers(Env)) {Env[[i]] <- raster::mask(Env[[i]], as(extent(extent_Europe), 'SpatialPolygons'))}
...
# run stacked SDMs
SSDM <- stack_modelling(setAlgorithms, Occ, Env, rep = 1, ensemble.thresh = 0,
