...
First, occurrence numbers.

```{r}
# load raw data
occ.raw <- read.csv(file=paste0(here::here(), "/data/Earthworm_occurrence_GBIF-sWorm-Edaphobase.csv"))

# load cleaned occurrence records 
occ.clean <- read.csv(file=paste0(here::here(), "/results/Occurrences_", Taxon_name, ".csv"))

# calculate number of records per datasource
full_join(occ.raw %>% group_by(datasource) %>% count(name="raw"), 
          occ.clean %>% group_by(datasource) %>% count(name="clean"))

# count occurrences per species & datasource
count.data <- occ.raw %>% group_by(datasource, species) %>% count() %>%
  pivot_wider(names_from = datasource, values_from = n) %>%
  mutate("TotalOcc" = rowSums(count.data[,2:4])
...
# plot total species' occurrences
plotOccRawGER <- ggplot(occ.clean, 
       aes(x=longitude, y=latitude, color=datasource))+ #, alpha=`Number of Records`
  #geom_polygon(data=bg.map)
  geom_point(cex=0.6)+
...
```{r plotOcc}
# load species list containing information on number of occurrence records in grid
...
plotOcc <- ggplot(occ.points %>% 
                    ...
                    summarize("Number of Records" = sum(`Number of Records`), .groups ="keep"), 
...
# pdf(paste0(here::here(), "/figures/GriddedOccurrences_", Taxon_name, ".pdf")); plotOcc; dev.off()

rm(plotOcc, occ.points)
```
...
rm(plotOccSpecies, occ.points.species)
```
...
for(temp.sp in speciesNames$SpeciesID){ try({
  ...
  temp.model <- mod_eval[mod_eval$tss==max(mod_eval$tss), "model"]
  temp.model
  
  # information about modelling (running time, background data)
  temp.details <- mod_eval[mod_eval$tss==max(mod_eval$tss), c("model", "tss", "time", "bg")]
  temp.details
...
var_imp <- var_imp %>%
  pivot_longer(cols=colnames(var_imp[,-1]), names_to = "model", values_to = "varImp")

# plot VIF
plotVarImp <- ggplot(data=var_imp, aes(x=varImp, y=Predictor, fill=model))+
  geom_bar(stat="identity")+
  xlab("Variable importance (Sum of scaled importances [0,10])")+
  theme_bw()
plotVarImp
