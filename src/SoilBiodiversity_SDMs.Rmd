...
data <- data.frame("NumCells_2km" = count_cuts, "Num_species"=NA, "Num_speciesID"=NA)
for(i in count_cuts){
   data[data$NumCells_2km==i,"Num_species"] <- speciesNames %>% filter(NumCells_2km>=i) %>% dplyr::select("Species_final") %>% unique() %>% count()
   data[data$NumCells_2km==i,"Num_speciesID"] <- speciesNames %>% filter(NumCells_2km>=i) %>% dplyr::select("SpeciesID") %>% unique() %>% count()
}
data

data2 <- data.frame("Records" = count_cuts, "Num_species"=NA, "Num_speciesID"=NA)
for(i in count_cuts){
   data2[data2$Records==i,"Num_species"] <- speciesNames %>% filter(Records>=i) %>% dplyr::select("Species_final") %>% unique() %>% count()
   data2[data2$Records==i,"Num_speciesID"] <- speciesNames %>% filter(Records>=i) %>% dplyr::select("SpeciesID") %>% unique() %>% count()
}
data2



write.csv(data, file=paste0(here::here(), "/results/NumSpecies_perOcc.csv"))

ggplot(data, aes(x=as.factor(NumCells_2km), y=Num_speciesID))+
...
count_data <- count_data %>% full_join(speciesNames[,c("Species", "Acc_name", "Species_final", "SpeciesID", #"NumCells_1km", 
									"NumCells_2km")], by=c("species" = "Species")) %>%
...
count_data[count_data$NumCells_2km >=5, "Included"] <- TRUE 
...
count_data <- count_data %>% full_join(speciesNames %>%
                                         dplyr::select(-NumCells_2km, -Group_name), by=c("species" = "Species", "Acc_name", "SpeciesID"))
count_data
