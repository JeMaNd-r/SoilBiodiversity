...
```{r numberOcc}
data.frame("No_species" = length(unique(speciesNames[speciesNames$NumCells_2km>0,"Species"])),
           "No_speciesID" = length(unique(speciesNames[speciesNames$NumCells_2km>0,"SpeciesID"])))
# >0: 155 (139 IDs), >=5: 81 (74 IDs), >=200: 18 (16 IDs); >=300: 15 (13 IDs)
...
for(sp in unique(speciesNames[speciesNames$NumCells_2km>=5,]$SpeciesID)){ try({
    ...
    best_bg <- mod_eval[mod_eval$tss == max(mod_eval$tss, na.rm=T) & !is.na(mod_eval$bg), "bg"]
