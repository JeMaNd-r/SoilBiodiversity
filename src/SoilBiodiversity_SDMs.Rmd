...
occ_points <- read.csv(file=paste0(here::here(), "/results/Occurrence_rasterized_", Taxon_name, ".csv"))
...
count_data <- occ_clean %>% group_by(datasource, species) %>% count() %>%
  pivot_wider(names_from = datasource, values_from = n) %>%
  full_join(species.list[,c("Species", "NumCells", "Records")], by=c("species" = "Species"))
count_data$TotalOcc <- rowSums(count_data[,2:4], na.rm=T)
