#- - - - - - - - - - - - - - - - - - - - - -#
#                                           #
#      Point occurrences to 10km grid       #
#          author: Romy Zeiss               #
#                                           #
#- - - - - - - - - - - - - - - - - - - - - -#
...
## load grid
r <- raster::raster("data/grid_10k_Germany.tif")
...
write.csv(occ_points, file=paste0(here::here(), "/results/Occurrence_rasterized_10km_", Taxon_name, ".csv"), row.names=F)

occ_points %>% pivot_longer(cols=(colnames(occ_points %>% dplyr::select(-x, -y, -year)))) %>% filter(!is.na(value))
# XX (1990: XX) records in total
