...
for(i in 1:length(stack_files)){
  ...
  names(temp_raster) <- gsub("_2.*", "", basename(stack_files[i]))
  
  temp_raster <- terra::mask(temp_raster, grid2k)
  
  Env <- c(Env, temp_raster)
...
# save raster
terra::writeRaster(Env, file="I:/eie/==PERSONAL/RZ SoilBON/SoilBiodiversity/results/EnvPredictor_2km.grd", overwrite=T)
